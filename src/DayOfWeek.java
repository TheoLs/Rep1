...
public class DayOfWeek {
	...
	int year;
		...
		System.out.println("Please input the date in a DD/MM/YYYY format"); // Telling the user, what format to input the date
		String[] input = scan.nextLine().split("/");						// Created a empty String group called input, so I could use .split 
		day = Integer.parseInt(input[0]);									// month is put in the input group as the first(0) element
		month = Integer.parseInt(input[1]);									// day is the second(1) element
		...
		m = (month +10) % 12; // calc month offset, Mar = 1, ...., Feb = 12 
		// month = 10 then = 8
		if(month <= 2) {
